# The main file which provides NRM support/integration with our VNE emulator code.
...
def _h_to_s(host_name):
    """ Converts 'host name' (our code convention) to 'string' (NRM code convention) 
    such that NRM code can understand it. Doing the additional -1 to comply with 
    NRM code's conventions. """
    return str(int(host_name[1:]) - 1)


def _s_to_h(s):
    """ Converts 'string' (NRM code convention) back to 'host name' (our code convention). """
    return 'h' + str(s + 1)


def _generate_substrate_graph_u_object():
    """ Generates and returns the substrate graph_u.Graph object (as per NRM 
    code convention) since NRM expects the substrate network represented in this
    graph_u.Graph object.
    This function shall take the data structures represented in my code, and 
    convert/create appropriate data structures so that NRM algorithms's code can 
    directly be called, hence providing direct integration with NRM. """
...
def _v_to_s(v):
    """ Convert from virtual host (in our code convention) to NRM's code convention. """
    return v - 1


def _s_to_v(s):
    """ Convert from NRM's code convention to virtual host (in our code convention). """
    return s + 1


def _generate_vnr_graph_u_object(vnr_num_hosts, vnr_cpu_reqs, vnr_link_reqs):
    """ Generates and returns the VNR graph_u.Graph object (as per NRM code convention) 
    since NRM expects the VNR represented in this graph_u.Graph object.
    This function shall take the data structures represented in my code, and 
    convert/create appropriate data structures so that NRM algorithms's code can 
    directly be called, hence providing direct integration with NRM. """
...
def get_ranked_hosts(vnr_num_hosts, vnr_cpu_reqs, vnr_link_reqs):
    ...
    # Creating the graph_u.Graph object (NRM code convention) for substrate network.
    substrate_graph_u = _generate_substrate_graph_u_object()

    # Creating the graph_u.Graph object (NRM code convention) for virtual network request.
    ...
    # the node ranking function of NRM can be called.
    nrm_ranked_virtual_nodes, nrm_ranked_substrate_nodes = nrm.node_rank(
        substrate_graph_u, vnr_graph_u, 1)

    print("\nnrm_ranked_virtual_nodes: ", nrm_ranked_virtual_nodes)
    print("\nnrm_ranked_substrate_nodes: ", nrm_ranked_substrate_nodes)

    # The results (nrm_ranked_virtual_nodes, nrm_ranked_substrate_nodes) obtained from
    # calling `nrm.node_rank()`, i.e. NRM's node ranking function are still in
    # NRM code's convention. They shall now be converted back to our code convention before
    # returning from this function.
    # Populating `ranked virtual hosts` in our code convention.
    ranked_virtual_hosts = []
    for vh in nrm_ranked_virtual_nodes:
        ranked_virtual_hosts.append(_s_to_v(vh))

    # Populating `ranked substrate hosts` in our code convention.
    ranked_substrate_hosts = []
    for sh in nrm_ranked_substrate_nodes:
